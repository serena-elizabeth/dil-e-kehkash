  ...
  return (
    <section id="comments" className="mt-12">
      ...
      {user ? (
        <div className="glass-card rounded-sm p-4 mb-8 flex flex-col gap-3">
          <textarea
            ...
            rows={3}
            className="w-full bg-transparent font-sans text-sm text-white/80 placeholder-white/20 resize-none outline-none"
          />
          <div className="flex items-center justify-between">
            <span className="text-xs text-white/20">{text.length}/{MAX_COMMENT_LENGTH}</span>
            <button
      ...
      <div className="flex flex-col gap-4">
        <AnimatePresence>
          {comments.length === 0 && (
            <p className="text-sm text-white/20 font-sans">No comments yet. Be the first.</p>
          )}
                ...
                <div className="flex items-center justify-between gap-2 mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {(isAdmin || user?.uid === comment.uid) && (
                      <button onClick={() => handleDelete(comment.id, comment.uid)} className="text-white/20 hover:text-red-400 transition-colors">
                        <FiTrash2 size={12} />
                      </button>
                    )}
                  </div>
                </div>
                <p className="text-sm text-white/70 font-sans leading-relaxed break-words">{comment.text}</p>
              </div>
            </motion.div>
          ))}
        </AnimatePresence>
